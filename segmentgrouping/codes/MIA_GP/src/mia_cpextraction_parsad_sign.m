function [idx]=mia_get_corner2(curve)
...
%       17th International Conference on Pattern Recognition, 2:791-794, August 2004.
    [dp] = linefit_Prasad_RDP_opt(curve);

    idx = [];
    for i=1:length(dp)
        dpp = dp{i}';
        idx{1,i} = polygon_extractconnectingpoints_sign(dpp);
    end
end
...
function cpm = mia_cpextraction_parsad_sign(imgbw)
    ...
    [dp] = linefit_Prasad_RDP_opt(edgelist);
    for i=1:length(dp)
        dpp = dp{i}';
...
function cp = polygon_extractconnectingpoints_sign(dp)
    nmdp = size(dp,2);
    l = 1;
    cp = {};
    s = zeros(1,nmdp);
